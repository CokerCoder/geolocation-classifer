...
for n in [100, 200, 300]:
    w2v_model = gensim.models.word2vec.Word2Vec(
        all_tokens, vector_size=n, window=8, min_count=1, sg=1, epochs=30, workers=cores)
    ...
    for sent in tqdm(train_tokens):  # for each tweet
        sent_vec = np.zeros(n)  # as word vectors are of zero length 300,
    ...
    for classifier in [
        ...
        print(f'{type(clf).__name__} F1 on N={n}： {f1:.4f}')
